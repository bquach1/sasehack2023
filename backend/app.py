...
def summary():
    ...
    pastSummary = summaryCollection.find_one(
        {
            "week": {
                "$eq": f"{date.today() - timedelta(((itteration - 1) + 7))}-{date.today() + timedelta(((8 - itteration)-7))}"
            }
        }
    )
    if pastSummary:
        week.append(
            {
                "role": "system",
                "content": f"Summary you gave the week before: {pastSummary['reply']}",
            }
        )
        print(pastSummary)
    else:
        print(
            f"{date.today() - timedelta(((itteration - 1) + 7))}-{date.today() + timedelta(((8 - itteration)-7))}"
        )
    ...
    for i in range(itteration):
        ...
        if result:
            ...
            dic = {
                "role": "user",
                "content": f"data for {day}, rating: {rating}/5, refection: {reflection}",
            }
            week.append(dic)
        else:
            week.append({"role": "user", "content": f"data for {day}: No data"})

    response = openai.ChatCompletion.create(model="gpt-3.5-turbo", messages=week)
    reply = response["choices"][0]["message"]["content"]
